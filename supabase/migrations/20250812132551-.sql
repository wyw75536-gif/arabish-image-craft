...
create policy if not exists "Device can select own api keys"
  on public.api_keys
  for select
  using (device_id = coalesce(public.request_header('x-device-id'), ''));

create policy if not exists "Device can insert own api keys"
  on public.api_keys
  for insert
  with check (device_id = coalesce(public.request_header('x-device-id'), ''));

create policy if not exists "Device can update own api keys"
  on public.api_keys
  for update
  using (device_id = coalesce(public.request_header('x-device-id'), ''))
  with check (device_id = coalesce(public.request_header('x-device-id'), ''));

create policy if not exists "Device can delete own api keys"
  on public.api_keys
  for delete
  using (device_id = coalesce(public.request_header('x-device-id'), ''));

-- Allow validation by bearer key passed in x-api-key header (without exposing rows)
create policy if not exists "API client can validate using x-api-key"
  on public.api_keys
  for select
  using (
    enabled = true
